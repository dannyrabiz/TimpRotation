...
control_data <- read.csv("control_file.csv")
for(i in 1:nrow(control_data)) {
  ...
  ggsave(paste0('2Clusters/', ID, '_k2.png'), plot = p, width = 10, height = 8, units = "in")
  
  # Assuming `dlpfc` is your SingleCellExperiment object and `spatial.cluster` is the column with cluster assignments
  cluster_assignments <- dlpfc$spatial.cluster
  
  # Creating a data frame with barcodes and their corresponding cluster assignments
  output_data <- data.frame(Barcode = colnames(dlpfc), Cluster = cluster_assignments)
  
  # Writing the data frame to a CSV file
  write.csv(output_data, paste0('2Clusters/',ID, '_k2_cluster_assignments.csv'), row.names = FALSE)
  }
