...
#gene_lists <- list(rownames(spe0), rownames(spe1))
...
spe2 <- spe1[common_genes, ]
spe3 <- spe1[common_genes, ]
...
sce.combined = cbind(spe0, spe1, deparse.level = 1)
...
ggplot(data.frame(reducedDim(sce.combined, "UMAP")), 
  ...
  geom_point(alpha = 0.5) +  # Adjust alpha for transparency
...
sce.combined = spatialCluster(sce.combined, use.dimred = "HARMONY", q = 9, nrep = 10000) #use HARMONY
...
p2 <- clusterPlot(sce_sample1, color = NA) +
  labs(title = "BayesSpace Clustering: Sample2") +
  theme_bw()
